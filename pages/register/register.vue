<template>
	<view class="container">
		...
		<view class="input-modualr">
			...
			<input v-model="password" placeholder-class="placeholder-class" class="input-text" placeholder="Enter your password"/>
		</view>
		<view class="input-modualr">
...
<script>
	export default {
		...
		onLoad(option) {
			if(uni.getStorageSync('user_invite')){
				this.invite = uni.getStorageSync('user_invite')
			}
		...
		methods: {
			...
			sumbit() {
				if(!this.account){
					uni.showModal({
						title: "Tip",
						content: "Please enter your registered account",
						confirmText: "confirm",
						showCancel: false,
					})
					return
			...
			getUserList() {
				...
				.then(res => {
					...
					if (res.data.code == 200) {
						...
						if(uni.getStorageSync('buy_url')){
							uni.redirectTo({
								url: uni.getStorageSync('buy_url')
							});
